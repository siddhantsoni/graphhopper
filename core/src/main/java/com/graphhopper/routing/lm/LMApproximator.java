...
public class LMApproximator implements WeightApproximator
{
    ...
    private final TIntObjectMap<VirtEntry> virtNodeMap;
                           ...
                           double factor, boolean reverse, int refreshCount )
    {
        ...
        virtNodeMap = new TIntObjectHashMap(graph.getNodes() - idxVirtNode, 0.5f, -1);
    ...
    public double approximate( final int queryNode )
    {
        ...
        if (counter == 0)
            lms.initActiveLandmarks(node, to, activeLandmarks, activeFromIntWeights, activeToIntWeights, reverse);
